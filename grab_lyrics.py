...
def write_dicts(genre, links):
    ...
    for link in links:
        if link.find(delimiter) > -1:
            splitted = link.split(delimiter)
            link = splitted[0]
            id = splitted[1]
            filename = id + ".txt"
        else:
            l = urllib.unquote(link)
            id = l[l.rfind("/")+1:].lower().replace("+","_").replace("'", "")
            filename = id + ".txt"
...
def write_normalized_counts():
    ...
    for filename in os.listdir(raw_counts_dir_name):
        ...
        ngrams = eval(contents[-1])
        for token, count in ngrams.iteritems():
            doc_freqs[token] += 1
        doc_count += 1
    ...
    for filename in os.listdir(raw_counts_dir_name):
        ...
        for token in result:
            log_int = (doc_count+1) / float(doc_freqs[token])
